...
import com.callrecorder.payamgostar.models.Model;
...
public class FileHelper {
    ...
    private static void createNoMedia(File dir) {
        try {
            File nomedia = new File(dir, ".nomedia");
            nomedia.createNewFile();
        } catch (IOException ex) {
            Logger.e(Constants.TAG, "Failed to create .nomedia file.");
        }
    }
    ...
    public static String getFilename(String phoneNumber) throws Exception {
        ...
        try {
            ...
            Logger.e(Constants.TAG, "Exception " + phoneNumber);
            e.printStackTrace();
        }
    ...
    public static void deleteAllRecords(Activity caller) {
        ...
        for (int i = 0; i < listOfFileNames.length; i++) {
            ...
            if (file2.exists() && !file2.getName().contains("nomedia")) {
                file2.delete();
            }
        }
    ...
    public static List<Model> listDir2(File f, Activity caller) {
        ...
        for (File file : files) {
            if (!file.getName().matches(Constants.FILE_NAME_PATTERN)) {
                Logger.d(Constants.TAG, String.format(
    ...
    public static void deleteFile(String fileName) {
        if (fileName == null)
            return;
        Logger.d(Constants.TAG, "FileHelper deleteFile " + fileName);
        try {
